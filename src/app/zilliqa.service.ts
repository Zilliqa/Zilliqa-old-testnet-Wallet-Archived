...
export class ZilliqaService {
  ...
  constructor(private http: HttpClient) {
    ...
    this.recentTxns = [];
    // @ts-ignore
    this.provider = new HTTPProvider(environment.API_URL || Constants.API_URL);
    this.walletData = {
      version: null,
      encryptedWalletFile: null
    };
   ...
   */
  initLib() {
    const nodeUrl = environment.API_URL || Constants.API_URL;
  ...
  checkEncryptedWallet(): boolean {
    ...
    if (this.walletData.encryptedWalletFile == null) return false;
    ...
    if (passphrase && passphrase.length >= 8) {
      ...
      scryptAsync(
        ...
        function(derivedKey) {
          ...
          let mac = sha3
            .digest(Buffer.concat([derivedKey.slice(16, 32), ciphertext]))
            .toString("hex");
  ...
  updateAccount(): Promise<any> {
    ...
    this.node.getBalance({ address: this.userWallet.address }, function(
      err,
      data
    ) {
      if (err || data.error) {
        ...
        deferred.resolve();
        console.log(
          `Couldn't fetch account details, balance: ` +
            that.userWallet.balance +
            `, nonce: ` +
            that.userWallet.nonce
        );
      } else {
        that.userWallet.balance = data.result.balance;
        that.userWallet.nonce = data.result.nonce;
        console.log(
          `Fetched account details successfully, balance: ` +
            that.userWallet.balance +
            `, nonce: ` +
            that.userWallet.nonce
        );
  ...
  sendPayment(payment: any): Promise<any> {
    ...
    const txCoreFields = {
      ...
      pubKey: this.getPublicKeyfromPrivateKey(),
      toAddr: payment.address,
      amount: new BN(payment.amount),
      gasPrice: new BN(payment.gasPrice),
      gasLimit: Long.fromNumber(payment.gasLimit)
    };

    const msg = util.encodeTransactionProto(txCoreFields);
    ...
    const tx = {
      ...txCoreFields,
      amount: txCoreFields.amount.toString(),
      gasPrice: txCoreFields.gasPrice.toString(),
      gasLimit: txCoreFields.gasLimit.toString(),
      data: "",
      code: "",
      signature
    };
    ...
    let that = this;
    this.provider
      .send(RPCMethod.CreateTransaction, tx)
      .then(res => {
        console.log("Result: ", res);
        // add to local cached list
        that.addLocalTxn({
          txnId: res.result.TranID,
          amount: tx.amount,
          toAddr: tx.toAddr
        });

        deferred.resolve({
          txId: res.result.TranID
        });
  ...
  createContract(codeStr, initParams, amount, gas): Promise<any> {
    ...
    this.updateAccount().then(() => {
      ...
      var txn = this.zilliqa.util.createTransactionJson(
        this.userWallet.privateKey,
        {
          version: 0,
      ...
      this.node.createTransaction(txn, function(err, data) {
        if (err || data.error) {
          ...
          let nonceStr = that.zilliqa.util
            .intToByteArray(that.userWallet.nonce, 64)
            .join("");
          ...
          console.log(
            "Contract should be deployed at address " + contractAddr + " soon."
          );
  ...
  getContractHistory(): Promise<any> {
    ...
    this.node.getSmartContracts({ address: this.userWallet.address }, function(
      err,
      data
    ) {
      if (err || data.error) {
  ...
  callTxnMethod(addr, method, amount, gas, params): Promise<any> {
    ...
    this.updateAccount().then(() => {
      // setup a dummy transaction
      var txn = that.zilliqa.util.createTransactionJson(
        that.userWallet.privateKey,
        {
          version: 0,
          nonce: +that.userWallet.nonce + 1,
          to: addr,
